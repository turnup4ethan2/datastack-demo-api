...
| `tags`            | string[]   | Tags assigned to the product.                      |
...
| `tags`            | string[] | No       | Tags to assign to the product. Defaults to `[]`. |
...
| `items`              | OrderItem[]   | Yes      | Line items on the order. Must be non-empty.             |
| `shipping_address`   | string        | Yes      | Shipping address as a single formatted string.          |
...
| Field               | Type    | Required | Description                              |
|---------------------|---------|----------|------------------------------------------|
| `product_id`        | string  | Yes      | Product being purchased.                 |
...
| `items`             | OrderItem[]  | Line items on the order.                                            |
...
Update the status of an order. Only admins can transition an order to `confirmed`, `shipped`, or `delivered`. Users may cancel their own orders while they are still in `pending` status.
