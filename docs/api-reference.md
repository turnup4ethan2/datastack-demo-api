...
| `gift_message` | `string \| null` | No | Message included with the order. Defaults to `null`; not returned in the response |
...
| `items` | `array[object]` | Line items with `product_id`, `quantity`, `unit_price_cents` |
