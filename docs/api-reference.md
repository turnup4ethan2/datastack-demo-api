...
DataStack core API — user management, product catalog, and order processing. API version `2.1.0`.
...
Query-parameter API keys are deprecated and are no longer accepted.
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `user_id` | string (path) | Yes | ID of the user to fetch |

**Response** — `200 OK`, a user object with the same fields as `POST /users`.
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `user_id` | string (path) | Yes | ID of the user to delete |
...
| `tags` | array of string | No | Tags to apply. Defaults to `[]` |
...
**Response** — `201 Created`, a product object with the same fields as `GET /products`.
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `product_id` | string (path) | Yes | ID of the product to fetch |

**Response** — `200 OK`, a product object with the same fields as `GET /products`.
...
An order item is an object with the fields:

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `product_id` | string | Yes | ID of the ordered product |
...
| `total_cents` | integer | Order total in USD cents, computed as the sum of `quantity * unit_price_cents` |
...
`priority_shipping` and `gift_message` are accepted on the request but are not returned in the order response.
...
curl -X POST "https://api.datastack.io/orders" \
  -H "Authorization: Bearer $DATASTACK_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"user_id":"usr_abc123","items":[{"product_id":"prod_001","quantity":2,"unit_price_cents":4999}],"shipping_address":"123 Main St, San Francisco, CA 94105","promo_code":"SPRING25","priority_shipping":true,"gift_message":"Happy birthday!"}'
```
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `order_id` | string (path) | Yes | ID of the order to fetch |

**Response** — `200 OK`, an order object with the same fields as `POST /orders`.
...
Update the status of an order; only admins can transition to `confirmed`, `shipped`, or `delivered`, and users may cancel their own `pending` orders.
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `order_id` | string (path) | Yes | ID of the order to update |
| `status` | string (body) | Yes | New status: `pending`, `confirmed`, `shipped`, `delivered`, or `cancelled` |
...
Report service liveness. No authentication required.
