...
> **Last updated: March 2026** — This document is kept in sync automatically by Devin.
...
| `tags` | `list[string]` | Tags assigned to the product |
...
| `tags` | `list[string]` | No | Tags to assign. Defaults to `[]` |
...
| `items` | `list[OrderItem]` | Yes | Line items in the order (see below) |
...
| `tracking_number` | `string` or `null` | Shipping tracking number, if assigned |
