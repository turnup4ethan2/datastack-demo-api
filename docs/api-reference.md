...
> API version `2.1.0`. This document is generated from the route definitions in `app/routes/`.
...
The header is mandatory — requests without it are rejected with `422`. Query-parameter API keys (`?api_key=`) are deprecated and no longer supported.
...
|-----------------|--------|----------|--------------------------------------|
| organization_id | string | Yes      | Query param — organization to filter |
...
| Field | Type   | Required | Description                     |
|-------|--------|----------|---------------------------------|
| tag   | string | No       | Query param — filter by one tag |
...
The order object is shared by all order endpoints:

| Field            | Type              | Description                                                                |
|------------------|-------------------|----------------------------------------------------------------------------|
| id               | string            | Order ID                                                                   |
| user_id          | string            | User the order belongs to                                                  |
| items            | array(order item) | Line items — see below                                                     |
...
| items             | array(order item) | Yes      | Line items                                               |
...
| gift_message      | string \| null    | No       | Message included with the order (default `null`)         |

`priority_shipping` and `gift_message` are accepted on input but are not part of the order response body.
...
Update the status of an order — only admins may transition to `confirmed`, `shipped` or `delivered`; users may cancel their own `pending` orders.
...
| 422  | Validation error (missing header or field) |
