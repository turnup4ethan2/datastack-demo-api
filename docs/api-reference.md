...
| `inventory_count` | `integer` | No | Current inventory (default: `0`) |
...
| `items` | `OrderItem[]` | Yes | List of line items (see below) |
...
| `items` | `OrderItem[]` | List of line items |
...
| `tracking_number` | `string\|null` | Shipping tracking number, if assigned |
| `created_at` | `string` | ISO 8601 timestamp |
