...
> **Last updated: May 2026** — This document is auto-generated by Devin from `app/routes/`.
...
Requests missing or with an invalid token receive `401 Unauthorized`.
...
| Field             | Type   | Description                              |
|-------------------|--------|------------------------------------------|
| `id`              | string | Newly created user ID.                   |
| `email`           | string | User email address.                      |
| `name`            | string | User display name.                       |
| `role`            | string | One of `admin`, `member`, `viewer`.      |
| `organization_id` | string | Organization the user belongs to.        |
...
Returns the created `Product`. See [List Products](#list-products) for the schema.
...
Returns a `Product`. See [List Products](#list-products) for the schema.
...
| `items`             | array<OrderItem>  | Yes      | Line items in the order. Must contain at least one item. |
...
| `tracking_number`  | string \| null    | Shipment tracking number once shipped.                                      |
| `created_at`       | string            | ISO-8601 UTC timestamp.                                                     |
...
Returns an `Order`. See [Create Order](#create-order) for the schema.
...
Returns the updated `Order`. See [Create Order](#create-order) for the schema.
