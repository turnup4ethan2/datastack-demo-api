...
> **API version: 2.1.0** — This document is generated and kept in sync with `app/routes/` by Devin.
...
The header is required on every request; requests without it are rejected with `422`.
The legacy `api_key` query parameter is deprecated and no longer accepted.
...
**Response** — `201 Created`, product object (same fields as [List Products](#list-products)).
...
**Response** — `200 OK`, product object (same fields as [List Products](#list-products)).
...
`priority_shipping` and `gift_message` are accepted on input but are not returned in the response.
...
**Response** — `200 OK`, order object (same fields as [Create Order](#create-order)).
...
Update the status of an order; only admins can transition to `confirmed`, `shipped`, or `delivered`, while users may cancel their own `pending` orders.
...
| 422  | Validation error (missing/invalid field or header)   |
