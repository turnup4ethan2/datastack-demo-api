...
An order item (`items[]`) has the fields:
...
| `total_cents` | integer | Sum of `quantity × unit_price_cents`, in USD cents |
...
Note: `priority_shipping` and `gift_message` are accepted on create but are not returned in the order response.
...
Updates the status of an order — only admins may transition to `confirmed`, `shipped`, or `delivered`; users may cancel their own `pending` orders.
