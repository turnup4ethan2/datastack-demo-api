...
**Response** — `201 Created` — a `User` object (see fields above)
...
**Response** — `201 Created` — a `Product` object (see fields above)
...
| `gift_message`      | `string`      | No       | Message to include with the order. Defaults to `null` |
...
**Response** — `200 OK` — an `Order` object (see fields above)
...
| 422  | Validation error (missing header or invalid body)    |
