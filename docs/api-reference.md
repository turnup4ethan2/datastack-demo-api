...
Requests missing this header are rejected with `422 Unprocessable Entity`.
...
Returns API liveness status. No authentication required.
...
**Response** — `201 Created` — [User](#user-object) object
...
**Response** — `200 OK` — [User](#user-object) object
...
Returns all products in the catalog, optionally filtered by tag.
...
**Response** — `201 Created` — [Product](#product-object) object
...
**Response** — `200 OK` — [Product](#product-object) object
...
| `promo_code`       | `string \| null` | Promo code applied                                                             |
...
| `items`             | `OrderItem[]` | Yes      | Line items (see [Order Item](#order-item-object)) |
...
`priority_shipping` and `gift_message` are accepted on creation but are not included in the returned Order object.
...
**Response** — `201 Created` — [Order](#order-object) object
...
**Response** — `200 OK` — [Order](#order-object) object
...
| `422` | Validation error (missing/invalid body, query param, or `Authorization` header) |
