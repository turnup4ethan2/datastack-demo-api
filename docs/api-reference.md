...
> **Last updated: June 2026** — This document is kept in sync with `app/routes/` automatically by Devin.
...
| `items`             | array of object  | Yes      | Line items in the order (see **Order item** below).|
| `shipping_address`  | string           | Yes      | Destination shipping address.                      |
| `promo_code`        | string           | No       | Promotional code to apply. Defaults to `null`.     |
...
| `items`            | array of object | Line items (see **Order item** above).                            |
...
| `id`               | string          | Unique order ID.                                                  |
| `user_id`          | string          | ID of the user who placed the order.                              |
| `items`            | array of object | Line items (see **Order item** under Create Order).               |
| `shipping_address` | string          | Destination shipping address.                                     |
| `status`           | string          | One of `pending`, `confirmed`, `shipped`, `delivered`, `cancelled`. |
| `total_cents`      | integer         | Order total in USD cents.                                         |
| `promo_code`       | string \| null  | Applied promo code, or `null`.                                    |
