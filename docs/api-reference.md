...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `email` | string (email) | Yes | Email address; validated |
...
**Response** — `201 Created`, user object (fields as in `GET /users`).
...
An order item object has the fields:
...
`priority_shipping` and `gift_message` are accepted on create but not returned in the order object.
