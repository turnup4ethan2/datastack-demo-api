...
The legacy `api_key` query parameter is deprecated and is no longer accepted.
...
Same fields as the user object in [List Users](#list-users).
...
Same fields as the product object in [List Products](#list-products).
...
| items              | array of order item   | Yes      | Line items in the order.                             |
...
| items             | array of order item | Line items in the order.                                       |
...
Note: `priority_shipping` and `gift_message` are accepted on the request but are not returned in the order object.
...
Same fields as the order object in [Create Order](#create-order).
