...
> **Version 2.1.0** — This document is kept in sync with `app/routes/` automatically.
...
Used in order request bodies and responses.
...
| `total_cents` | `integer` | Order total in USD cents (`sum(quantity * unit_price_cents)`) |
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `user_id` | `string` | Yes | Ordering user |
| `items` | `array<OrderItem>` | Yes | Line items (see Order Item object) |
...
| `gift_message` | `string` | No | Gift message to include with the order. Default `null` |
