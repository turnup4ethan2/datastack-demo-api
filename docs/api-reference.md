...
The deprecated `api_key` query parameter is no longer supported.
...
**Response** — `201 Created`, the created product object (same fields as List Products).
...
| `items`             | array[order item] | Yes      | Line items. See the order item shape above.          |
...
| `gift_message`      | string \| null    | No       | Message included with the order. Defaults to `null`. |
...
**Response** — `200 OK`, an order object (same fields as Create Order).
...
**Response** — `200 OK`, the updated order object (same fields as Create Order).
