...
The previous query-parameter API key (`?api_key=...`) pattern is deprecated and no longer supported.
...
| `tags`            | array<string> | No       | Tags for the product. Default `[]`.  |
...
Returns an order object (see [Create Order](#create-order) response schema).
...
Returns the updated order object (see [Create Order](#create-order) response schema).
