...
| `role` | `string` | No (default: `"member"`) | User role: `"admin"`, `"member"`, or `"viewer"` |
...
| `inventory_count` | `integer` | No (default: `0`) | Initial inventory count |
| `tags` | `list[string]` | No (default: `[]`) | Tags to associate with the product |
...
| `items` | `list[OrderItem]` | Yes | Line items for the order (see below) |
