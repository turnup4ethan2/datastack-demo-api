...
> API version `2.1.0`. This document is kept in sync with `app/routes/` by an automated Devin session.
...
The query-parameter API key pattern (`?api_key=...`) is deprecated and no longer accepted.
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `email` | string (email) | Yes | Email address; must be a valid address |
...
Same fields as the user object in `POST /users`.
...
An order item has the following shape, used in both requests and responses:
...
Place a new order. Inventory is reserved immediately and payment is captured asynchronously, so the order is returned in `pending` status. `total_cents` is computed as the sum of `quantity * unit_price_cents` across items.
...
| `items` | array of order item | Yes | Items to order |
| `shipping_address` | string | Yes | Destination address |
| `promo_code` | string \| null | No | Promotional code to apply |
| `priority_shipping` | boolean | No | Request expedited shipping. Defaults to `false` |
| `gift_message` | string \| null | No | Message to include with the shipment |
...
| `items` | array of order item | Ordered items |
...
Same fields as the order object in `POST /orders`.
...
| `tracking_number` | string \| null | No | Carrier tracking number to record |
