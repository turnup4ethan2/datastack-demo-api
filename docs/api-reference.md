...
API keys passed as a query parameter (`?api_key=...`) are deprecated and no longer accepted.
...
| `items`             | array of object | Yes      | Order items (see shape above).                                 |
...
| `gift_message`      | string \| null  | No       | Gift message. Defaults to `null`. Not returned in the response. |
...
| `items`            | array of object | Order items.                                                                 |
...
**Response** — `200 OK`, order object (same fields as `POST /orders`).
