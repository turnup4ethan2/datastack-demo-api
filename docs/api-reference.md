...
An order `status` is one of `pending`, `confirmed`, `shipped`, `delivered`, `cancelled`.
...
| `items`            | array of object | Line items with `product_id`, `quantity`, `unit_price_cents`       |
...
**Response** — `200 OK`. Same fields as [Create Order](#create-order).
