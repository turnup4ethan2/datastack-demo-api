...
> **Last updated: July 2026** — Maintained automatically by Devin to reflect the current implementation in `app/routes/`.
...
| promo_code        | string     | No       | Optional promotional code (default `null`)   |
...
| items            | object[]   | Order items (`product_id`, `quantity`, `unit_price_cents`)      |
