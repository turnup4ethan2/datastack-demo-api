...
The DataStack API is organized around three resources: **Users**, **Products**, and **Orders**. All endpoints accept and return JSON.
...
Requests without a valid token return `401 Unauthorized`. The previous `api_key` query-parameter auth has been removed.
...
| `organization_id` | string | Yes      | Organization the new user belongs to.                |
...
Same shape as the Create User response.
...
Array of product objects with the following fields:
...
| `inventory_count` | integer        | No       | Initial stock count. Defaults to `0`.                |
...
Same shape as the List Products response items.
...
| `items`             | array[OrderItem] | Yes      | One or more line items. See `OrderItem` below.                     |
| `shipping_address`  | string           | Yes      | Full shipping address.                                             |
| `promo_code`        | string           | No       | Optional promo code.                                               |
| `priority_shipping` | boolean          | No       | Whether to use expedited shipping. Defaults to `false`.            |
...
| `tracking_number`   | string \| null   | Carrier tracking number once shipped, otherwise `null`.                  |
| `created_at`        | string           | ISO 8601 timestamp.                                                      |
...
Same shape as the Create Order response.
...
Update the status of an order. Only admins can transition an order to `confirmed`, `shipped`, or `delivered`. Users may transition their own `pending` orders to `cancelled`.
...
| `tracking_number` | string | No       | Carrier tracking number. Typically set on the `shipped` transition. |
...
Same shape as the Create Order response, with the updated `status` and `tracking_number`.
