...
> **Last updated: March 2026** — This document is kept in sync automatically via Devin.
...
| `role` | `string` | No (default: `"member"`) | One of `"admin"`, `"member"`, `"viewer"` |
...
| `inventory_count` | `integer` | No (default: `0`) | Initial inventory quantity |
| `tags` | `array[string]` | No (default: `[]`) | List of tags |
...
| `items` | `array[OrderItem]` | Yes | List of items to order (see below) |
| `shipping_address` | `string` | Yes | Shipping address |
| `promo_code` | `string` | No | Promotional code to apply |
| `priority_shipping` | `boolean` | No (default: `false`) | Whether to use priority shipping |
