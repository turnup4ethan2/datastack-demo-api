...
API version `2.1.0`. This document is kept in sync with `app/routes/` by Devin.
...
The header is required; requests without it are rejected with `422`/`401`. API keys passed as the
`api_key` query parameter are no longer supported.
...
**Response** — `201 Created`, a user object (`id`, `email`, `name`, `role`, `organization_id`, `created_at`).
...
**Response** — `200 OK`, a user object (fields as in `POST /users`).
...
| Field | Type   | Required | Description                                  |
|-------|--------|----------|----------------------------------------------|
| `tag` | string | No       | Query parameter. Return only products carrying this tag. |
...
**Response** — `201 Created`, a product object (fields as in `GET /products`).
...
**Response** — `200 OK`, a product object (fields as in `GET /products`).
...
An order item object has the fields `product_id` (string), `quantity` (integer), and
`unit_price_cents` (integer, USD cents).

Order `status` is one of `pending`, `confirmed`, `shipped`, `delivered`, `cancelled`.

### POST /orders

Place a new order. Inventory is reserved immediately and payment is captured asynchronously, so the
order is returned in `pending` status. `total_cents` is computed as the sum of
`quantity * unit_price_cents` across `items`.
...
| `items`             | array of order item   | Yes      | Line items; each has `product_id`, `quantity`, `unit_price_cents`. |
...
| `gift_message`      | string \| null        | No       | Gift message. Defaults to `null`. Not echoed in the response. |
...
| `status`           | string              | See status values above; `pending` on creation. |
...
**Response** — `200 OK`, an order object (fields as in `POST /orders`).
...
Update the status of an order. Only admins can transition an order to `confirmed`, `shipped`, or
`delivered`; users may cancel their own `pending` orders.
...
**Response** — `200 OK`, the updated order object (fields as in `POST /orders`).
