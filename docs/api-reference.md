...
The header is required on every request; query-parameter API keys are no longer supported.
...
| Field | Type   | Required | Description              |
|-------|--------|----------|--------------------------|
| `tag` | string | No       | Only return products carrying this tag |
...
| `tags`            | array of string | No       | Tags to apply (default `[]`)              |
...
An order item (`items[]`) has the following shape in both requests and responses:
...
| `items`             | array of item     | Yes      | Ordered line items                                 |
| `shipping_address`  | string            | Yes      | Destination address                                |
| `promo_code`        | string \| null    | No       | Promotional code (default `null`)                  |
| `priority_shipping` | boolean           | No       | Request expedited shipping (default `false`)       |
| `gift_message`      | string \| null    | No       | Message to include with the shipment (default `null`) |
...
| `items`            | array of item  | Ordered line items                                                           |
