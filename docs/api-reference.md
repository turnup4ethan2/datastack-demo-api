...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `tag` | `string` (query) | No | Return only products carrying this tag |
...
| `inventory_count` | `integer` | Units on hand |
| `tags` | `array[string]` | Tags attached to the product |
| `created_at` | `string` | ISO 8601 timestamp |
...
| `inventory_count` | `integer` | No | Units on hand; defaults to `0` |
...
**Response** — `201 Created`, same fields as the product object above.
...
The order object is returned by all order endpoints:

| Field | Type | Description |
|-------|------|-------------|
| `id` | `string` | Order ID |
| `user_id` | `string` | User who placed the order |
| `items` | `array[object]` | Line items: `product_id` (`string`), `quantity` (`integer`), `unit_price_cents` (`integer`, USD cents) |
...
| `items` | `array[object]` | Yes | Line items; each requires `product_id` (`string`), `quantity` (`integer`), `unit_price_cents` (`integer`, USD cents) |
...
| `gift_message` | `string` | No | Message included with the shipment; defaults to `null` |
...
**Response** — `201 Created`, order object. Note that `priority_shipping` and `gift_message` are accepted on input but are not returned in the order object.
...
| `tracking_number` | `string` (body) | No | Carrier tracking number; defaults to `null` |
