...
Creates a new user. Requires the caller to have the `admin` role on the organization.
...
**Response** — `201 Created` — User object (see fields under [GET /users](#get-users))
...
**Response** — `200 OK` — User object (see fields under [GET /users](#get-users))
...
| `tags`            | `string[]` | Tags attached to the product.                |
| `created_at`      | `string`   | ISO 8601 timestamp.                          |
...
| `inventory_count` | `integer`  | No       | Initial stock level. Defaults to `0`.         |
...
**Response** — `201 Created` — Product object (see fields under [GET /products](#get-products))
...
**Response** — `200 OK` — Product object (see fields under [GET /products](#get-products))
...
| `shipping_address` | `string`        | Free-form shipping address.                                                 |
| `status`           | `string`        | One of `pending`, `confirmed`, `shipped`, `delivered`, `cancelled`.         |
| `total_cents`      | `integer`       | Order total in USD cents (sum of `quantity * unit_price_cents` over items). |
...
| `items`             | `OrderItem[]` | Yes      | Line items (see [OrderItem](#order-object)).                 |
| `shipping_address`  | `string`      | Yes      | Free-form shipping address.                                  |
...
> `priority_shipping` and `gift_message` are accepted on input but are not echoed back in the Order object.
...
**Response** — `201 Created` — [Order object](#order-object)
...
**Response** — `200 OK` — [Order object](#order-object)
