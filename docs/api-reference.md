...
Requests without a valid `Authorization` header are rejected. Query-parameter API keys (`?api_key=...`) are no longer supported.
...
|-------------------|----------|----------|------------------------------------------|
| `organization_id` | `string` | Yes      | ID of the organization to list users for |
...
| Field | Type     | Required | Description                                  |
|-------|----------|----------|----------------------------------------------|
| `tag` | `string` | No       | Return only products that include this tag   |
...
Used in the `items` field of order requests and responses.
...
| `items`            | `OrderItem[]`    | Line items (see [Order item object](#order-item-object))                     |
...
| `items`             | `OrderItem[]` | Yes      | Line items (see [Order item object](#order-item-object))               |
...
| `gift_message`      | `string`      | No       | Gift message to include with the shipment. Defaults to `null`          |

`priority_shipping` and `gift_message` are accepted on creation but are not echoed back in the order object.
...
Returns an [Order object](#order-object) with `status` set to `pending` and `tracking_number` set to `null`.
