...
API version `2.1.0`. Base path is the root of the API host.
...
Query-parameter API keys are no longer supported.
...
**Response** — `201 Created`, a user object (same fields as List Users).
...
**Response** — `200 OK`, a user object (same fields as List Users).
...
**Response** — `201 Created`, a product object (same fields as List Products).
...
**Response** — `200 OK`, a product object (same fields as List Products).
...
An order object has these fields:

| Field              | Type             | Description                                                              |
|--------------------|------------------|--------------------------------------------------------------------------|
| `id`               | string           | Order ID.                                                                |
| `user_id`          | string           | User who placed the order.                                               |
| `items`            | array of object  | Line items: `product_id` (string), `quantity` (integer), `unit_price_cents` (integer, USD cents). |
...
| `promo_code`       | string or null   | Promo code applied, if any.                                              |
| `tracking_number`  | string or null   | Carrier tracking number, if any.                                         |
| `created_at`       | string           | ISO 8601 timestamp.                                                      |
...
`priority_shipping` and `gift_message` are accepted on input but are not returned on the order object.
