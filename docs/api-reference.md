...
> **API version: 2.1.0** — This document is generated from `app/routes/` by Devin.
...
The header is required on every request; query-parameter API keys are deprecated and no longer accepted.
...
|-------|------|-------------|
| `id` | `string` | Order ID |
| `user_id` | `string` | User the order belongs to |
...
Places a new order. Inventory is reserved immediately and payment is captured asynchronously, so the order is returned in `pending` status.
...
| `items` | `array<object>` | Yes | Line items, each with `product_id` (`string`), `quantity` (`integer`), `unit_price_cents` (`integer`, USD cents) — all required |
| `shipping_address` | `string` | Yes | Destination address |
| `promo_code` | `string` | No | Promo code to apply. Defaults to `null` |
| `priority_shipping` | `boolean` | No | Request expedited shipping. Defaults to `false`. Not echoed in the response |
| `gift_message` | `string` | No | Message to include with the shipment. Defaults to `null`. Not echoed in the response |
...
Updates the status of an order. Only admins may transition an order to `confirmed`, `shipped`, or `delivered`; users may cancel their own `pending` orders.
