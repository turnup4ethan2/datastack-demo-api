...
> **Base URL:** `https://api.datastack.io/v2`
...
```bash
curl -X GET "https://api.datastack.io/v2/users?organization_id=org_xyz" \
...
```bash
curl -X POST "https://api.datastack.io/v2/users" \
  -H "Authorization: Bearer <token>" \
...
```bash
curl -X GET "https://api.datastack.io/v2/users/usr_abc123" \
  -H "Authorization: Bearer <token>"
```
...
```bash
curl -X DELETE "https://api.datastack.io/v2/users/usr_abc123" \
  -H "Authorization: Bearer <token>"
```
...
```bash
curl -X GET "https://api.datastack.io/v2/products?tag=featured" \
  -H "Authorization: Bearer <token>"
```
...
| `inventory_count` | integer | No | Initial inventory level. Defaults to `0` |
| `tags` | array of strings | No | Product tags. Defaults to `[]` |
...
```bash
curl -X POST "https://api.datastack.io/v2/products" \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "Widget Pro",
...
```bash
curl -X GET "https://api.datastack.io/v2/products/prod_001" \
  -H "Authorization: Bearer <token>"
```
...
| `items` | array of `OrderItem` | Yes | Line items (see below) |
...
| `gift_message` | string | No | Optional gift message to include |
...
```bash
curl -X POST "https://api.datastack.io/v2/orders" \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{
    "user_id": "usr_abc123",
...
```bash
curl -X GET "https://api.datastack.io/v2/orders/ord_001" \
  -H "Authorization: Bearer <token>"
```
...
```bash
curl -X PATCH "https://api.datastack.io/v2/orders/ord_001/status" \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{
    "status": "shipped",
