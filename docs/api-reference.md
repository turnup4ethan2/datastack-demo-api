...
API version `2.1.0`. Examples below use `https://api.datastack.io` as the base URL — substitute your deployment's host (locally, `http://localhost:8000`).
...
Query-parameter API keys are deprecated and no longer accepted.
...
| `items` | array of item | Ordered items (`product_id`, `quantity`, `unit_price_cents`) |
...
| `items[].product_id` | string | Yes | Product to order |
| `items[].quantity` | integer | Yes | Units of the product |
| `items[].unit_price_cents` | integer | Yes | Price per unit in USD cents |
...
| `gift_message` | string | No | Message to include with the shipment. Defaults to `null` |
...
Note: `priority_shipping` and `gift_message` are accepted on input but are not returned on the order object.
...
Fetches a single order by ID. Non-admin users can only fetch their own orders.
...
| 403 | Authenticated but not permitted (e.g. non-admin status transition) |
